...
It is important to note that these resolutions are resolutions are congruent with the resolutions used in the LPW paper \cite{zhang_max-planck-institut_nodate}. This is important for the comparison of the results.
When scaling an image there is always an image interpolation done to find the best approximation for the new intensity value $I$. The interpolation used in this thesis is the bilinear interpolation.\cite{gonzalez_bilinear_nodate} This is a linear interpolation in the x and y axis of the intensity values and solves equation \ref{eq:bilinear}. let $Q_{11}=(x_1,y_1)$, $Q_{12}=(x_1,y_2)$, $Q_{21}=(x_2,y_1)$ and $Q_{22}(x_2,y_2)$ be the four surrounding points. The intensity value $I$ at $(x,y)$ is then calculated by equation \ref{eq:bilinear}. The point P at $(x,y)$ is the point of interest. 
...
\subsubsection{Histogram equalisation \cite{noauthor_opencv_nodate}}
Another important aspect of preprocessing the frames is using Histogram equalization. This has the effect of increasing the contrast of the image. For this task Contras Limited Adaptive Histogram Equalization (CLAHE) is used. CLAHE is an Histogram Equalization method that has the benefit that it is adaptive to the local contrast of the image. This is very useful if the contrast of the image is not uniform. By using Histogram Equalization on the frames, noise is added to the image. This can be dealt with by using a low pass filter, like an Gaussian filter for example.
...
After calculating the gradient Canny edge detection is used to refine the edges. Canny edge detection is a multi step algorithm that uses a hysteresis thresholding to filter the edges. The result of the edge detection is a one pixel thick binary edge map.  All edges are now possible candidates for the pupil contour. 
\subsubsection{Sobel Operators \cite{gonzalez_sharpening_nodate}}
...
\end{center}
    ...
    \subsubsection{Canny Edge Detection \cite{gonzalez_canny_nodate}} 
    The Canny Edge Detection is used to recieve single edge points form the gradient magnitude image.  
    This algorithm can summarized in four steps: 
    \begin{enumerate}
...
\subsection{Haar like feature detection \cite{viola_rapid_nodate}}
...
\subsubsection{Haar-Like Feature for pupil detection \cite{swirski_robust_2012}}
...
\newpage
\subsection{Random Sample Consensus (RANSAC) \cite{derpanis_overview_nodate}}
...
\newpage
\subsection{Active Contouring \cite{vondracek_image_2018}}
...
\subsubsection{Active Contouring without Edges (ACWE) \cite{vondracek_image_2018}}
